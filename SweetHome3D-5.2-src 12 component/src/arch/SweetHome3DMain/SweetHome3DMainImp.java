...
public class SweetHome3DMainImp  extends HomeApplication implements ISweetHome3DMainImp
{
  ...
  private static class FileContentManagerWithRecordedLastDirectories extends FileContentManager {
    ...
    private final Class<? extends SweetHome3DMainImp> mainClass;

    public FileContentManagerWithRecordedLastDirectories(UserPreferences preferences, 
                                                         Class<? extends SweetHome3DMainImp> mainClass) {
      super(preferences);
          ...
          if (ev.getType() == CollectionEvent.Type.ADD) {
            if (ev.getItem().getName() != null 
              ...
              if (OperatingSystem.isMacOSXLionOrSuperior()
                  ...
                  && _arch.OUT_MainToMacConfig.isWindowFullScreen(getHomeFrame(home))) {
  ...
  public void start(String [] args) {
	...
	private SweetHome3DMainArch _arch;
	

    public SweetHome3DMainImp (){
	...
	*/
	   ...
	   */
	  protected void inits(final String [] args) {
	    
	    _arch.OUT_MainToInitialization.initialization(args);
...
//	      };
//	    });
	    _arch.OUT_MainToBuildHome.buildHome();
	    _arch.OUT_MainToJava3DErrorListener.addComponent3DRenderingErrorObserver();
	    _arch.OUT_MainToGetUserPreferences.getUserPreferences();
	    ...
	    _arch.OUT_MainToSwingEnsurance.initLookAndFeel();
	    try {
	    ...
	    if (OperatingSystem.isMacOSX()) {
	      ...
	      _arch.OUT_MainToMacConfig.bindToApplicationMenu(this);
	    }

	    // Run everything else in Event Dispatch Thread
	    EventQueue.invokeLater(new Runnable() {
	      public void run() {
	        SweetHome3DMainImp.this.start(args);
	      }
	    });
	  }
